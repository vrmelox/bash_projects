...
echo "$($PSQL "SELECT major FROM students FULL JOIN majors ON students.major_id = majors.major_id WHERE major IS NOT NULL (student_id IS NULL OR first_name ILIKE '%ma%') ORDER BY major")";
